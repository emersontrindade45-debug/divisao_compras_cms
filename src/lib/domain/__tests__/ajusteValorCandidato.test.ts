...
describe("valorUnitarioEfetivo", () => {
  it("usa o valor ajustado quando existe", () => {
    expect(valorUnitarioEfetivo({ valorUnitario: 15000, valorUnitarioAjustado: 100 })).toBe(100);
  });

  it("cai no valor original da fonte quando não há ajuste", () => {
    expect(valorUnitarioEfetivo({ valorUnitario: 15000, valorUnitarioAjustado: null })).toBe(15000);
  });
});
